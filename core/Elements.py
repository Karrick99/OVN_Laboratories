...
class Line:
    ...
    def __init__(self, label, length):
        ...
        self.Rs = const.Rs
        self.df = const.df
        self.Pbase = const.h*self.f*self.Bn
    ...
    def nli_generation(self):
        ...
        self.eta_nli = (16 / (27 * np.pi)) * np.log10(
            (np.pi ** 2 / 2) * (self.beta2 * self.Rs ** 2 / self.alpha) * const.Nch ** (2 * self.Rs / self.df)) \
                  * ((self.gamma ** 2 / (4 * self.alpha * self.beta2)) * 1 / (self.Rs ** 3))
        return (const.Pch**3)*self.eta_nli*(self.n_amplifiers)*const.Bn

    def optimized_launch_power(self):

       return ((self.noise_figure*self.Leff*self.Pbase)/(2*self.Bn*self.eta_nli))**(1/3)
...
class Network:
    def __init__(self):
        ...
        with open("Lab7/nodes_full_flex_rate.json", "r") as in_file:
    ...
    def calculate_bit_rate(self, path, strategy):
        ...
        if strategy == 'flex_rate':
            # print("gsnr=")
            # print(GSNR)
            # print("limite sup 200Gbps=")
            # print(10 * ((erfcinv((8 / 3) * 1e-3)) ** 2) * 32 / 12.5)

            if GSNR < 2 * ((erfcinv(2e-3)) ** 2) * (32 / 12.5):
                return 0
            elif 2 * ((erfcinv(2e-3)) ** 2) * 32 / 12.5 <= GSNR < (14 / 3) * (
                    (erfcinv((3 / 2) * 1e-3)) ** 2) * 32 / 12.5:
                return 100e9
            elif (14 / 3) * ((erfcinv((3 / 2) * 1e-3)) ** 2) * 32 / 12.5 <= GSNR < 10 * (
                    (erfcinv((8 / 3) * 1e-3)) ** 2) * 32 / 12.5:
                return 200e9
        ...
        if strategy == 'shannon':
            ...
            return 2 * const.Rs * np.log2(1 + (const.Bn / const.Rs) * GSNR)
